...
- **IME-композиция** (REL-4) — при активном методе ввода (CJK/вьетнамский/диакритика)
  свитчер встаёт в stand-down (детект типа TIS-источника); dead-key даёт пустой ввод
  и не буферится. Конвертация только по завершённому слову обычными символами.
- **Транслит latin→cyrillic** (FR-23) — обратное направление неоднозначно (y→й,
  e→е), best-effort; ru→lat детерминирован.
- Словари ~10.4k RU / ~4.5k EN словоформ + частотные списки (2.5k/2.8k) для tiebreak;
  морфология (суффикс/стем-модель) обобщает на неперечисленные формы. Консервативно
  (≤0.5% ложняков, гейт в eval-harness). Растут импортом (FR-22) и обучением на откатах.
- **Веб-пароли** (REL-6) — закрыто доменным stand-down: добавь host в Настройки →
  Словари → «Домены без автоконверта». Плюс per-app режимы (auto/shadow/off + форс-раскладка).
